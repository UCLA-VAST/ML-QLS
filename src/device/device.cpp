...
void Device::constructQubit(){
    ...
    _vvASAP.clear();
    _vvASAP.resize(_nQubit, vector<double_t>(_nQubit, 0));
}
...
void Device::printDevice(){
    ...
    fprintf(stdout, "       - #Edge            : %lu\n", _vEdge.size());
    fprintf(stdout, "       ------------------------------------------\n");
    fprintf(stdout, "       - Edge list\n");
    for ( i = 0; i < _vEdge.size();  ++i ){
...
void Device::calAPSP(){
    _vvASAP.clear();
    _vvASAP.resize(_nQubit, vector<double_t>(_nQubit, MAX_DOUBLE));
    // unsigned_t i, j, k;/
    // for(i = 0; i < _vEdge.size(); ++i){
